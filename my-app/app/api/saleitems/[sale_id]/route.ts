...
export async function GET(req: NextRequest, { params }) {
    ...
    const saleItemsResult = await DBProvider.dbSelect(tableName, where, sort) as IQueryResult<ISaleItemQuery, ISaleItem>;

    const itemIds = saleItemsResult.result.map(x => x.item_id).filter((v,i,a) => a.indexOf(v) === i);
    const itemsResult = await DBProvider.dbSelect('items', [{column: 'id', operator: Operator.IsIn, value: itemIds} as ICondition]) as IQueryResult<IItemQuery, IItem>;
...
export async function DELETE(req: NextRequest, { params }) {  
    const saleitemResults = await DBProvider.dbSelect(tableName, [{column: 'id', operator: Operator.Equals, value: params.id} as ICondition]) as IQueryResult<ISaleItemQuery, ISaleItem>;

    const saleResults = await DBProvider.dbSelect('sales', [{column: 'id', operator: Operator.Equals, value: saleitemResults.result[0].sale_id} as ICondition]);
    const sale = saleResults.result[0];

    const deleted = await DBProvider.dbDelete(tableName, params.id);
